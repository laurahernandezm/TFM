...
def compute_metrics(accs, names):
    results = pd.DataFrame(columns=["TP", "FP", "FN", "IDs"])
    i = 0
    for name, acc in zip(names, accs):
        logging.info('Computing metrics for %s...', name)
        df = acc.events[acc.events['Type'] != "RAW"].drop_duplicates(
            subset=["Type", "OId", "HId"]
        )
        matched_hs = set(df[df['Type'] == 'MATCH'].HId.unique())
        matched_gts = set(df[df['Type'] == 'MATCH'].OId.unique())

        fps_hs = set(df[df['Type'] == 'FP'].HId.unique()) - matched_hs
        missed_gts = set(df[df['Type'] == 'MISS'].OId.unique()) - matched_gts

        results.loc[name] = {
            "TP": len(matched_gts),
            "FP": len(fps_hs),
            "FN": len(missed_gts),
            "IDs": len(matched_hs) - len(matched_gts)}
        i+=1
...
def main():
    ...
    results = compute_metrics(accs, names)
